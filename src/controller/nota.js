function gerarPDF() {
    ...
    // Calcular o valor da taxa para mostrar no PDF
    ...
    // Adiciona a taxa apenas se for maior que 0
    if (valorTaxa > 0) {
        valores.push(["Taxa Cartão (6,77%)", `R$ ${valorTaxa.toFixed(2)}`]);
    }
    
    valores.push(["Total", `R$ ${getValue('total')}`]);
    ...
    // Cores tons de cinza
const primaryColor = [90, 90, 90];   // Cabeçalho
const accentColor = [90, 90, 90];    // Destaques
const textColor = [15, 15, 15];      // Texto
const boxColor = [245, 245, 245];    // Fundo das caixas
...
function calcularTotal() {
    ...
    // Só duas opções: Cartão ou Dinheiro/Pix
    ...
    // Aplica taxa apenas para Cartão
    if (formaPagamento === "Cartão") {
